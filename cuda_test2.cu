    ...
    do { \
        ...
        if (status != NVML_SUCCESS) { \
            fprintf(stderr, "NVML Error at %s:%d: %s\n", \
                    __FILE__, __LINE__, nvmlErrorString(status)); \
            /* Attempt to shut down NVML gracefully */ \
            if (status != NVML_ERROR_UNINITIALIZED) { nvmlShutdown(); } \
            return 1; \
        } \
    } while (0)
...
void print_separator(const char* title) {
    printf("\n%s", title);
    for (size_t i = 0; i < 60 - strlen(title); i++) printf("=");
    printf("\n");
}
...
int main() {
    ...
    if (cublasCreate(&cublasH) != CUBLAS_STATUS_SUCCESS) return 1;
    ...
    printf("   Warming up...\n");
    for (int i=0; i<5; i++) {
        cublasHgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, TENSOR_SIZE, TENSOR_SIZE, TENSOR_SIZE,
                    &alpha, d_A, TENSOR_SIZE, d_B, TENSOR_SIZE, &beta, d_C, TENSOR_SIZE);
    }
    ...
    double flops = 2.0 * (double)TENSOR_SIZE * TENSOR_SIZE * TENSOR_SIZE * ITERATIONS;
    double tflops_estimate = flops / (elapsed_s * 1e12);

    printf("Compute test PASSED in %.2fs (~%.1f TFLOPS FP16)\n", elapsed_s, tflops_estimate);
